...
async def tmdb(bot: Client, message: Message):
    ...
    # Determine the first_id and last_id based on the arguments provided
    if len(args) == 0:
        first_id = 1  # Default value if no argument is given
        last_id = message.id
    elif len(args) == 1:
        first_id = int(args[0])
        last_id = message.id
        ...
        if last_id <= first_id:
            await message.reply(text="The second value (last_id) must be greater than the first value (first_id).")
            return
    else:
        await message.reply(text="Invalid number of arguments. Use /tmdb <first_id> [last_id].")
        return

    try:
        # Handle the logic using first_id and last_id
        start_message = (
        ...
        files = await get_messages(message.chat.id, first_id, last_id)
                    ...
                    series_doc = {
                        ...
                        "seasons": [{
                            ...
                            "episodes": [{
                                ...
                                "title": re.sub(r'[^a-zA-Z0-9 ]', '', final.get("name")),
