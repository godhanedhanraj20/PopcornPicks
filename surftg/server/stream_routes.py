...
async def login_route(request):
    ...
    # Allow login if the username and password match the valid users or admin
    if (username in Telegram.USERS and Telegram.USERS[username] == password) or (username == Telegram.ADMIN_USERNAME and password == Telegram.ADMIN_PASSWORD):
        ...
        session['session_id'] = secrets.token_hex(16)  # Unique session identifier
        active_sessions.setdefault(username, []).append(session['session_id'])  # Store all active session ids
...
async def logout_route(request):
    ...
    if username and session_id:
        # Remove the current session id from active sessions for the user
        active_sessions.get(username, []).remove(session_id)
        # Clear the session
        session.invalidate()
